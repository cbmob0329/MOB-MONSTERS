...
window.MOBMON_EXPLORATION=function(api){
  ...
  function leave(){stop();field=null;busy=false;view=null;}
  ...
  async function depart(area,deep){
    ...
    await warp('ロボに乗って出発！',()=>{field=next;const s=api.state();s.screen='story';s.story.current={season:D.seasons.find(s=>s.loop===(deep?2:1)&&s.areas.includes(area)).season,area,floor:deep?2:1,areaNo:1};save();},fieldAssets(next));
  }
  ...
  function pool(area,deep){const list=D.monsters.filter(m=>String(m.firstArea).startsWith(area)&&m.sourceCategory==='normal'&&m.species!=='ボス'&&(deep||D.rankOrder.indexOf(m.rank)<=5));return list.length?list:D.monsters.filter(m=>m.firstArea==='草原'&&m.sourceCategory==='normal');}
  function elitePool(area){return D.monsters.filter(m=>String(m.firstArea).startsWith(area)&&m.sourceCategory==='elite');}
  function valid(f,p,r=4){return p.x>=r&&p.x<=f.size-r&&p.y>=r&&p.y<=f.size-r&&!f.obstacles.some(o=>Math.abs(o.x-p.x)<o.r+r&&Math.abs(o.y-p.y)<o.r+r&&(o.x-p.x)**2+(o.y-p.y)**2<(o.r+r)**2);}
  ...
  function draw(){if(!field||!view||!view.robot.isConnected)return;const f=field,v=view,u=v.unit;
    ...
    const sector=f.floor===4?'BOSS AREA':`SECTOR ${Math.min(2,Math.floor(f.player.x/100))+1} / ${Math.min(2,Math.floor(f.player.y/100))+1}`;if(v.sector.textContent!==sector)v.sector.textContent=sector;
    ...
    const radarKey=(t?.id||'none')+'|'+busy;if(v.radarKey!==radarKey){v.radarKey=radarKey;v.radar.disabled=!t||busy;v.radar.classList.toggle('detected',!!t);v.radarLabel.textContent=!t?'探索中':t.kind==='portal'?(f.floor===4?'エリアから出る':'次の階へ'):t.kind==='chest'?'開ける！':'バトル！';v.hint.textContent=t?(t.name||(t.kind==='chest'?'宝箱を発見！':'ワープホールを発見！')):'光る対象に近づいて、レーダーでアクション';}
  }
  ...
  async function interact(explicit){if(busy||!field)return;const e=explicit?.kind?explicit:target();if(!e)return;keys.clear();resetStick();
    ...
    if(field!==f)return;
    api.battle({mode:'exploration',encounterKind:e.kind,background:window.MOBMON_ASSET_MAP.battleAsset(f.area,e.kind),area:f.area,floor:f.floor,title:`${f.area}${f.deep?'深層':''} AREA ${f.floor}`,enemyRoster:roster,useFullParty:true,soulDrop:true,guaranteedSoul:e.kind==='boss'?e.name:null,onWin:()=>{if(e.kind==='boss'){progress().cleared[key(f.area,f.deep)]=true;api.complete(f.area,f.deep);save();}},onReturn:(win,fled)=>{
