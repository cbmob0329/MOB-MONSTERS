...
$releaseName = 'MOB-MONSTERS-v0030-PORTRAIT-BATTLE'
...
foreach ($name in @('smoke.js','ui_runtime_smoke.js','asset_contract.js','regression_v0030.cjs','update929_contract.cjs','battle_v0030.cjs','battle-v0030-results.json','performance_v0029.cjs','performance-v0029.json','fixtures')) {
    Copy-Item -LiteralPath (Join-Path $projectRoot "tests\$name") -Destination (Join-Path $releaseRoot 'tests') -Recurse -Force
}
Get-ChildItem -LiteralPath (Join-Path $projectRoot 'tests\screenshots') -Filter '030-*.png' | Copy-Item -Destination (Join-Path $releaseRoot 'tests\screenshots') -Force
