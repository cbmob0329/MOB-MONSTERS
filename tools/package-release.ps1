...
$releaseName = 'MOB-MONSTERS-v0028-ADVENTURE-BATTLE'
...
foreach ($name in @('smoke.js','ui_runtime_smoke.js','asset_contract.js','update912_browser.cjs','update912_contract.cjs','update912_features.cjs','update912-results.json')) {
    Copy-Item -LiteralPath (Join-Path $projectRoot "tests\$name") -Destination (Join-Path $releaseRoot 'tests') -Force
}
Get-ChildItem -LiteralPath (Join-Path $projectRoot 'tests\screenshots') -Filter '028-*.png' | Copy-Item -Destination (Join-Path $releaseRoot 'tests\screenshots') -Force
