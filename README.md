# MOB MONSTERS v0.3.6 — 仲間集め支援と施設の操作配置

パーティー3体未満の敵数制限、序盤ソウル率アップ、ナレーション修正、ラボとお城の画面内ボタン配置を追加しました。[変更点と検証](docs/RELEASE_v0036.md)。

以下は過去の更新履歴です。

# MOB MONSTERS v0.3.5 — 会話・冒険演出と初回草原

リリスの吹き出し・文字送り、ナレーションの分離、出発・帰還・Lvアップ演出、初回草原の敵調整を追加しました。[変更点と検証](docs/RELEASE_v0035.md)。

以下は過去の更新履歴です。

# MOB MONSTERS v0.3.4 — テスト用オート戦闘と習得Lv表示

戦闘中のAUTO切り替えと、ソウルレコード育成時の技・習得Lv表示を追加しました。[変更点と検証](docs/RELEASE_v0034.md)。

以下は過去の更新履歴です。

# MOB MONSTERS v0.3.3 — ソウル軽量化・交換・冒険Lv・オープニング

ソウル一覧のページ分割、ソウルラボの変換機能、施設の表示修正、敵の名前・Lv表示、冒険レベル設定とオープニングを追加しました。[変更点・レベル表・検証](docs/RELEASE_v0033.md)。

以下は過去の更新履歴です。

# MOB MONSTERS v0.3.0 — 下部の正方形パーティーと撃破演出

戦闘の味方表示を下部のメイン4体＋スーパーサブ2体の正方形画像に統一し、敵撃破時の演出を追加しました。[変更点と検証](docs/RELEASE_v0030.md)。

以下は過去の更新履歴です。

# MOB MONSTERS v0.2.9 — 戦闘の対象固定と探索軽量化

戦闘画面・対象選択・探索中の帰還制御・ボス融合・SP長押し育成を更新しました。[変更点と検証](docs/RELEASE_v0029.md)、[MOB STORYとの技の照合・素材一覧](docs/SKILL_COMPARISON_v0029.md)をご覧ください。
