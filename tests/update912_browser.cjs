...
(async()=>{
 ...
 await page.route('**/js/exploration.js*',route=>route.fulfill({contentType:'text/javascript',body:fs.readFileSync('js/exploration.js','utf8').replace('return{render,leave,stop,selector,generate,valid,deepUnlocked};','return{render,leave,stop,selector,generate,valid,deepUnlocked,field:()=>field,interact,animateMonster};')}));
 ...
 await page.waitForTimeout(1200);await page.screenshot({path:'tests/screenshots/028-title.png'});
 ...
 await page.locator('.facility.story').click();await page.waitForTimeout(2200);await page.screenshot({path:'tests/screenshots/028-destinations.png'});assert(await page.locator('[data-destination]').first().isVisible());await page.locator('[data-destination]').first().click();assert(await page.locator('#deepDepth').isDisabled());await page.locator('#normalDepth').click();await page.locator('#answerNo').click();assert.equal(await page.locator('.field-map').count(),0);
 ...
 await page.waitForTimeout(900);await page.screenshot({path:'tests/screenshots/028-field.png'});
 ...
 for(const [width,height] of [[320,568],[375,667],[390,844],[430,932]]){
  ...
  assert(layout.scroll<=1,JSON.stringify(layout));assert(layout.padBottom<=layout.navTop);assert(Math.abs(layout.center-width/2)<2);assert(layout.mapBottom<layout.padTop);await page.screenshot({path:`tests/screenshots/028-mobile-${width}.png`});
 }
 ...
 await page.locator('.encounter-overlay').waitFor();assert(await page.evaluate(()=>document.querySelector('#app').inert));await page.waitForTimeout(450);await page.screenshot({path:'tests/screenshots/028-encounter.png'});
 ...
 assert.equal(await page.evaluate(()=>window.__test.battle().enemyRoster[0].name),contactName);assert.equal(await page.locator('.ally-stage-v5').count(),1);assert.equal(await page.locator('.ally-hud-v3').count(),4);await page.waitForTimeout(2000);await page.screenshot({path:'tests/screenshots/028-battle.png'});
 ...
 await page.screenshot({path:'tests/screenshots/028-boss.png'});
 ...
 await page.locator('.boss-encounter').waitFor();await page.waitForTimeout(550);await page.screenshot({path:'tests/screenshots/028-boss-encounter.png'});await page.waitForFunction(()=>!!window.__test.battle());
 ...
 await page.screenshot({path:'tests/screenshots/028-boss-clear.png'});
 ...
 await page.waitForTimeout(350);const slime=await page.locator('.enemy-art-v3 img').boundingBox();assert(slime.width<=390&&slime.height<=160,JSON.stringify(slime));assert.equal(await page.locator('.ally-stage-v5').count(),1);await page.screenshot({path:'tests/screenshots/028-slime.png'});
