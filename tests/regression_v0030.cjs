...
(async()=>{
 ...
 await page.waitForTimeout(1200);await page.screenshot({path:'tests/screenshots/030-title.png'});
 ...
 await page.locator('.facility.story').click();await page.waitForTimeout(2200);await page.screenshot({path:'tests/screenshots/030-destinations.png'});assert(await page.locator('[data-destination]').first().isVisible());await page.locator('[data-destination]').first().click();assert(await page.locator('#deepDepth').isDisabled());await page.locator('#normalDepth').click();await page.locator('#answerNo').click();assert.equal(await page.locator('.field-map').count(),0);
 ...
 await page.waitForTimeout(900);await page.screenshot({path:'tests/screenshots/030-field.png'});
 ...
 for(const [width,height] of [[320,568],[375,667],[390,844],[430,932]]){
  ...
  assert(layout.scroll<=1,JSON.stringify(layout));assert(layout.padBottom<=layout.navTop);assert(Math.abs(layout.center-width/2)<2);assert(layout.mapBottom<layout.padTop);await page.screenshot({path:`tests/screenshots/030-mobile-${width}.png`});
 }
 ...
 await page.locator('.encounter-overlay').waitFor();assert(await page.evaluate(()=>document.querySelector('#app').inert));await page.waitForTimeout(450);await page.screenshot({path:'tests/screenshots/030-encounter.png'});
 ...
 assert.equal(await page.evaluate(()=>window.__test.battle().enemyRoster[0].name),contactName);assert.equal(await page.locator('.ally-stage-v5').count(),0);assert.equal(await page.locator('.main-card-v30').count(),4);await page.waitForTimeout(2000);await page.screenshot({path:'tests/screenshots/030-battle.png'});
 ...
 await page.screenshot({path:'tests/screenshots/030-boss.png'});
 ...
 await page.locator('.boss-encounter').waitFor();await page.waitForTimeout(550);await page.screenshot({path:'tests/screenshots/030-boss-encounter.png'});await page.waitForFunction(()=>!!window.__test.battle());
 ...
 await page.screenshot({path:'tests/screenshots/030-boss-clear.png'});
 ...
 await page.waitForTimeout(350);const slime=await page.locator('.enemy-art-v3 img').boundingBox();assert(slime.width<=390&&slime.height<=160,JSON.stringify(slime));assert.equal(await page.locator('.ally-stage-v5').count(),0);await page.screenshot({path:'tests/screenshots/030-slime.png'});
