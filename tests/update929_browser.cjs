...
(async()=>{
 ...
 await page.route('**/js/exploration.js*',route=>route.fulfill({contentType:'text/javascript',body:fs.readFileSync('js/exploration.js','utf8').replace('return{render,leave,stop,selector,generate,valid,deepUnlocked,active:()=>!!field};','return{render,leave,stop,selector,generate,valid,deepUnlocked,active:()=>!!field,field:()=>field,interact,animateMonster};')}));
 await page.route('**/js/game.js*',route=>route.fulfill({contentType:'text/javascript',body:fs.readFileSync('js/game.js','utf8').replace('renderBattle();runBattle();','renderBattle();if(!window.__skipBattleLoop)runBattle();').replace(/applyTestV6\(\);\r?\nrender\(\);/,'window.__test={exploration,state:()=>state,battle:()=>battle,awardBattleRewards,endBattleToScreen,finishBattle,startBattle};\napplyTestV6();\nrender();')}));
 await page.goto('http://127.0.0.1:4173');

 const gestureChecks=await page.evaluate(()=>{const app=document.querySelector('#app'),target=document.querySelector('.game-title');const events=['contextmenu','selectstart','dragstart','dblclick','gesturestart','gesturechange','gestureend'].map(type=>{const event=new Event(type,{bubbles:true,cancelable:true});target.dispatchEvent(event);return event.defaultPrevented;});const pinch=new Event('touchmove',{bubbles:true,cancelable:true});Object.defineProperty(pinch,'touches',{value:[{},{}]});target.dispatchEvent(pinch);const scroll=new Event('touchmove',{bubbles:true,cancelable:true});Object.defineProperty(scroll,'touches',{value:[{}]});target.dispatchEvent(scroll);const input=document.createElement('input');app.appendChild(input);const editing=new Event('selectstart',{bubbles:true,cancelable:true});input.dispatchEvent(editing);input.remove();return {events,pinch:pinch.defaultPrevented,scroll:scroll.defaultPrevented,editing:editing.defaultPrevented,selection:getComputedStyle(target).userSelect};});
 assert(gestureChecks.events.every(Boolean));assert(gestureChecks.pinch);assert(!gestureChecks.scroll);assert(!gestureChecks.editing);assert.equal(gestureChecks.selection,'none');
 const cdp=await page.context().newCDPSession(page);await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x:140,y:170},{x:240,y:170}]});await cdp.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[{x:100,y:170},{x:280,y:170}]});await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});assert.equal(await page.evaluate(()=>visualViewport.scale),1);
 assert.equal(await page.locator('.title-menu button').count(),3);assert(await page.locator('#continueGame').isDisabled());
 await page.waitForTimeout(1200);await page.screenshot({path:'tests/screenshots/029-title.png'});
 ...
 await page.locator('.facility.story').click();await page.waitForTimeout(2200);await page.screenshot({path:'tests/screenshots/029-destinations.png'});assert(await page.locator('[data-destination]').first().isVisible());await page.locator('[data-destination]').first().click();assert(await page.locator('#deepDepth').isDisabled());await page.locator('#normalDepth').click();await page.locator('#answerNo').click();assert.equal(await page.locator('.field-map').count(),0);
 await page.locator('[data-destination]').first().click();await page.locator('#normalDepth').click();await page.locator('#answerYes').click();await page.waitForTimeout(4600);
 assert.equal(await page.locator('.map-entity.enemy').count(),7);
 assert.equal(await page.locator('.obstacle').count(),72);
 await page.waitForTimeout(900);await page.screenshot({path:'tests/screenshots/029-field.png'});
 ...
 for(const [width,height] of [[320,568],[375,667],[390,844],[430,932]]){
  ...
  assert(layout.scroll<=1,JSON.stringify(layout));assert(layout.padBottom<=layout.navTop);assert(Math.abs(layout.center-width/2)<2);assert(layout.mapBottom<layout.padTop);await page.screenshot({path:`tests/screenshots/029-mobile-${width}.png`});
 }
 ...
 assert(await page.evaluate(()=>{const x=window.__test.exploration,f=x.generate('草原',false,1),e=f.entities.find(e=>e.kind==='enemy');Object.assign(e,{x:150,y:150,motion:'rest',motionLeft:1,hopPhase:0,angle:0});x.animateMonster(e,f,.2);if(e.x!==150||e.y!==150||e.hop!==0)return false;e.motion='hop';e.motionLeft=2;x.animateMonster(e,f,.1);if(e.x<=150||e.hop<=0)return false;for(let i=0;i<40;i++)x.animateMonster(e,f,.05);return e.motion==='rest';}));
 const speedStart=await page.evaluate(()=>({y:window.__test.exploration.field().player.y,t:window.__test.exploration.field().elapsed}));
 ...
 const speed=await page.evaluate(start=>{const f=window.__test.exploration.field();return (start.y-f.player.y)/(f.elapsed-start.t);},speedStart);assert(speed>25&&speed<=31,'robot speed '+speed);
 ...
 await page.locator('.encounter-overlay').waitFor();assert(await page.evaluate(()=>document.querySelector('#app').inert));await page.waitForTimeout(450);await page.screenshot({path:'tests/screenshots/029-encounter.png'});
 const frozen=await page.evaluate(()=>window.__test.exploration.field().player.y);await page.keyboard.press('ArrowUp');assert.equal(await page.evaluate(()=>window.__test.exploration.field().player.y),frozen);
 await page.evaluate(()=>window.__test.exploration.interact());assert.equal(await page.locator('.encounter-overlay').count(),1);
 await page.waitForFunction(()=>!!window.__test.battle());assert.equal(await page.locator('.encounter-overlay').count(),0);assert(!(await page.evaluate(()=>document.querySelector('#app').inert)));
 assert.equal(await page.evaluate(()=>window.__test.battle().enemyRoster[0].name),contactName);assert.equal(await page.locator('.ally-stage-v5').count(),1);assert.equal(await page.locator('.portrait-hud-v29').count(),4);await page.waitForTimeout(2000);await page.screenshot({path:'tests/screenshots/029-battle.png'});
 ...
 const next=async()=>{await page.evaluate(()=>{const x=window.__test.exploration;x.interact(x.field().entities.find(e=>e.kind==='portal'));});await page.locator('#answerYes').click();await page.waitForTimeout(4600);};
 ...
 await page.screenshot({path:'tests/screenshots/029-boss.png'});
 ...
 await page.locator('.boss-encounter').waitFor();await page.waitForTimeout(550);await page.screenshot({path:'tests/screenshots/029-boss-encounter.png'});await page.waitForFunction(()=>!!window.__test.battle());
 ...
 await page.screenshot({path:'tests/screenshots/029-boss-clear.png'});
 ...
 await page.waitForTimeout(350);const slime=await page.locator('.enemy-art-v3 img').boundingBox();assert(slime.width<=390&&slime.height<=160,JSON.stringify(slime));assert.equal(await page.locator('.ally-stage-v5').count(),1);await page.screenshot({path:'tests/screenshots/029-slime.png'});
 ...
 await page.emulateMedia({reducedMotion:'reduce'});assert(await page.evaluate(()=>{const x=window.__test.exploration,f=x.generate('草原',false,1),e=f.entities.find(e=>e.kind==='enemy');Object.assign(e,{motion:'hop',motionLeft:2,hopPhase:0});x.animateMonster(e,f,.1);return e.hop===0;}));
