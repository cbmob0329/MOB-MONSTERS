...
window.MOBMON_EXPLORATION=function(api){
  ...
  function leave(){if((field?.introWins||mainField?.introWins)>0){progress().grassIntroComplete=true;save();}stop();field=null;mainField=null;caveField=null;busy=false;view=null;}
  ...
  async function warp(label,fn,paths=[],kind='warp'){busy=true;stop();api.lock(true);try{await window.MOBMON_PRESENT.transition(label,paths,()=>{api.lock(false);fn();if(field)render(document.querySelector('#screen'));},kind);}finally{api.lock(false);api.close();busy=false;}}
  async function depart(area,deep){
    ...
    await warp('ロボに乗って出発！',()=>{mainField=null;caveField=null;field=next;const s=api.state();s.screen='story';s.story.current={season:campaign.entry(area,deep).season,area,floor:deep?2:1,areaNo:1};save();},fieldAssets(next),'depart');
  }
  ...
  function smallParty(){return new Set(api.state().party.slice(0,4).filter(id=>api.state().owned.some(m=>m.uid===id))).size<3;}
  function beginnerPool(){const all=pool('草原',false),rank=Math.min(...all.map(m=>D.rankOrder.indexOf(m.rank)));return all.filter(m=>D.rankOrder.indexOf(m.rank)===rank);}
  function monster(f,kind='enemy',p=null){let m=choose(f.intro?beginnerPool():pool(f.area,f.deep,f.inside));if(kind==='elite')m=f.area==='草原'?choose(D.monsters.filter(m=>['enemy/10.png','enemy/13.png'].includes(m.image))):choose(elitePool(f.area).length?elitePool(f.area):pool(f.area,f.deep));if(kind==='boss')m=D.monsters.find(m=>m.name===D.areaBoss[f.area]?.[f.deep?'2':'1']);return{...p||position(f),id:globalThis.crypto?.randomUUID?.()||('entity_'+Date.now().toString(36)+Math.random().toString(36).slice(2)),kind,level:f.intro?3:campaign.level(f.area,f.deep,f.floor,kind,f.inside),name:m.name,src:m.image,angle:Math.random()*Math.PI*2,age:0};}
  ...
  function generate(area,deep,floor){
    ...
    const f={area,deep,floor,size,intro:area==='草原'&&!deep&&floor===1&&(smallParty()||!progress().grassIntroComplete&&!cleared(area,false)),introWins:0,camera:{x:0,y:0},player:{x:size/2,y:size-8},walls:[],obstacles:[],entities:[],respawns:[],elapsed:0,portalPrompt:false};
  ...
  function render(root){
    ...
    root.querySelector('#leaveField').onclick=async()=>{if(await ask('エリアから帰還しますか？','現在の探索マップはリセットされます。獲得した報酬は保持されます。')){await warp('おかえりなさい！',()=>{leave();api.go('home');},[],'return');}};
  ...
  async function interact(explicit){if(busy||!field)return;const e=explicit?.kind?explicit:target();if(!e)return;keys.clear();resetStick();
    ...
    if(e.kind==='portal'){busy=true;const yes=await ask(field.floor===4?'エリアから出ますか？':'次の階に進みますか？',field.floor===4?'獲得したソウルと一緒に帰還します。':`AREA ${field.floor+1}へワープします。`);busy=false;if(!yes)return;if(field.floor===4){await warp('おかえりなさい！',()=>{leave();api.go('home');},[],'return');}else {if(field.intro){progress().grassIntroComplete=true;save();}const next=generate(field.area,field.deep,field.floor+1);await warp('次のAREAへ！',()=>{mainField=null;caveField=null;field=next;},fieldAssets(next));}return;}
    ...
    stop();busy=true;const f=field;const roster=[{name:e.name,level:e.level}];if(e.kind==='enemy')for(let i=1,n=smallParty()?1:f.intro?(f.introWins?1+Math.floor(Math.random()*2):1):1+Math.floor(Math.random()*4);i<n;i++)roster.push({name:choose(f.intro?beginnerPool():pool(f.area,f.deep,f.inside)).name,level:f.intro?3:campaign.level(f.area,f.deep,f.floor,'enemy',f.inside)});
    try{await encounter(e,roster);}catch(error){busy=false;render(document.querySelector('#screen'));toast('戦闘の準備をやり直してください');return;}
    if(field!==f)return;
    api.battle({mode:'exploration',encounterKind:e.kind,background:window.MOBMON_ASSET_MAP.battleAsset(f.area,e.kind),area:f.area,floor:f.floor,title:`${f.area}${f.deep?'深層':''} ${f.inside?site(f.area).name:'AREA '+f.floor}`,enemyRoster:roster,useFullParty:true,soulDrop:true,beginnerSupport:!!f.intro&&smallParty(),beginnerSoulBonus:f.area==='草原'&&!f.deep&&(smallParty()||f.intro)?.2:0,guaranteedSoul:e.kind==='boss'?e.name:null,onWin:()=>{if(f.intro)f.introWins++;if(e.kind==='elite'){(api.state().quests||={}).eliteGrass=api.state().quests.eliteGrass||f.area==='草原';save();}if(e.kind==='boss'){progress().cleared[key(f.area,f.deep)]=true;api.complete(f.area,f.deep);save();}},onReturn:(win,fled)=>{
